...
from dataloader import getTestingData, view_dataset, removePoints, setXYorigin
from dataloader import transform as pts_transforms
        ...
        with torch.no_grad():
            ...
            for i in range(len(final_results)):
                ...
                if len(pts[i]) > 0:
                    ...
                    pts[i] = pts[i][..., :2]
                    pts[i][...,0:1] = (pts[i][...,0:1])*x_scale
                    pts[i][...,1:2] = (-pts[i][...,1:2])*y_scale
                    pts[i] = np.concatenate(pts[i][0], axis=0) # (1, 17, 2) => (34)
                    ...
                    boxes.insert(i, [])
            pts = np.stack(pts, axis=0) #convert list to array
            temp = []
            for i in range(len(pts)):
                temp.append(pts_transforms(setXYorigin(removePoints(pts[i],[0,1,2,3],2),2), x_scale, 2))
                
            pts = np.stack(temp, axis=0)
...
if __name__ == '__main__':
    ...
    # import copy
    loader = getTestingData(1, ["TS1"], True)
    for sample in loader:
        images = sample['images'].cpu().detach().numpy()
        _2d_dots = sample['_2d_dots']
        _3d_dots = sample['_3d_dots']
        # plt.imshow(image[0])
        pts = hhrnet.predict(images)
        view_dataset(_2d_dots,pts,1000,2,False)
        break
